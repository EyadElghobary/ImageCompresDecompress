...
if __name__ == '__main__':
    ...
    com_decom = input("Choose c/d [c -> Compress | d -> Decompress] : ")
    if com_decom == 'c':
        loss_level = int(input("Loss [between 0-255] : "))
        file_name = input("File Name: ")
        mirror = MIRROR_IMG
        compress = Compressor(file_name, loss_level)
        compress.run()
        print("Compressing: " + file_name)
        print("Mirror : " + str(mirror))
    decompress = Decompressor(file_name + '.qdt')
    decompress.run()

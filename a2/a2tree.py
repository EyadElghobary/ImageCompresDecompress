...
class QuadTreeNodeInternal(QuadTreeNode):
    ...
    def convert_to_pixels(self, width: int, height: int) -> List[List[int]]:
        ...
        if width % 2 == 0:
            ...
            left, right = w, w + 1
        bl = self.children[0].convert_to_pixels(left, down)
        br = self.children[1].convert_to_pixels(right, down)
        tl = self.children[2].convert_to_pixels(left, up)
        tr = self.children[3].convert_to_pixels(right, up)
        ...
        for i in range(4):
            if lst[start] != '':
                if lst[start] == 'E':
                    ...
                    self.children[i] = QuadTreeNodeLeaf(int(lst[start]))
                start += 1
            else:
                node = QuadTreeNodeInternal()
                start += node.restore_from_preorder(lst, start)
    ...
    def mirror(self) -> None:
        ...
        """
        # TODO
        first_copy = deepcopy(self.children[0])
        second_copy = deepcopy(self.children[1])
        first_copy2 = mirror_help(first_copy)
        second_copy2 = mirror_help(second_copy)
        self.children[2] = first_copy2
        self.children[3] = second_copy2
...
class QuadTree:
    ...
    def _build_tree_helper(self, pixels: List[List[int]]) -> QuadTreeNode:
        ...
        """
        # TODO: complete this method
        if pixels[0] == [] and len(pixels) == 1:
            return QuadTreeNodeEmpty()
        elif len(pixels[0]) == 1 and len(pixels) == 1:
            return QuadTreeNodeLeaf(pixels[0][0])
        else:
            node = QuadTreeNodeInternal()
            colour_values = standard_deviation_and_mean(pixels)
            if colour_values[0] <= self.loss_level:
                return QuadTreeNodeLeaf(round(colour_values[1]))
            matrix = self._split_quadrants(pixels)
            children = []
            for sublist in matrix:
                children.append(self._build_tree_helper(sublist))
    ...
    def _split_quadrants(pixels: List[List[int]]) -> List[List[List[int]]]:
        ...
        if bottom == 0:
            ...
            for lst in pixels[:bottom]:
                lst1[0].append(lst[:left])
                lst1[1].append(lst[left:right])

        for lst in pixels[bottom:top]:
            lst1[2].append(lst[:left])
            lst1[3].append(lst[left:right])
        return lst1
...
def mirror_help(node: QuadTreeNode) -> QuadTreeNode:
    if isinstance(node, QuadTreeNodeLeaf):
        return node
    elif isinstance(node, QuadTreeNodeEmpty):
        return node
    elif isinstance(node, QuadTreeNodeInternal):
        first, second = deepcopy(node.children[0]), deepcopy(node.children[1])
        third, fourth = deepcopy(node.children[2]), deepcopy(node.children[3])
        node.children[0] = mirror_help(third)
        node.children[1] = mirror_help(fourth)
        node.children[2] = mirror_help(first)
        node.children[3] = mirror_help(second)
        return node
    raise RuntimeError
...
def maximum_loss(original: QuadTreeNode, compressed: QuadTreeNode) -> float:
    ...
    if isinstance(compressed, QuadTreeNodeLeaf) and\
            isinstance(original, QuadTreeNodeInternal):
        lst = standard_dev_getter(original)
        return standard_deviation_and_mean([lst])[0]
    elif isinstance(compressed, QuadTreeNodeEmpty) and\
            isinstance(original, QuadTreeNodeEmpty):
        return float(0)
    elif isinstance(compressed, QuadTreeNodeLeaf) and\
            isinstance(original, QuadTreeNodeLeaf):
        return float(0)
    else:
        max_list = []
        for i in range(4):
            result = maximum_loss(original.children[i], compressed.children[i])
            max_list.append(result)
        return max(max_list)


def standard_dev_getter(node: QuadTreeNode) -> List[int]:
    if isinstance(node, QuadTreeNodeLeaf):
        return [node.value]
    elif isinstance(node, QuadTreeNodeEmpty):
        return []
        ...
        for child in node.children:
            result = standard_dev_getter(child)
            lst.extend(result)
        return lst
    raise RuntimeError
